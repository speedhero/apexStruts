...
public class ComDisplayAction extends DispatchAction {
    ...
    public ActionForward addData(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        if(data!=null&&data!=""){
            if(colums.length>0){
                ...
                try{
                    for(Map columInfo:columInfos){
                        field    =  columInfo.get("FFieldCode").toString().toUpperCase().trim();
                        editType = columInfo.get("FFieldType").toString().trim();
                        ...
                        if(dataMap.get(field)==null) {
                            ...
                            fieldValue=dataMap.get(field).toString().trim();
                        }

                        if("1".equalsIgnoreCase(editType)) { //1|文本类型;2|数值类型;3|日期类型;4|大数据类型;{
                            sqlValue=sqlValue+" '"+fieldValue+"' ,";
                        }else if("2".equalsIgnoreCase(editType)) {
                            if("".equalsIgnoreCase(fieldValue)){
                                sqlValue=sqlValue+" null ,";
                    ...
                    for(int j=0;j<datalist.size();j++){
                        ...
                        try{
                            for(Map columInfo:columInfos){
                                ...
                                String fieldVal="";
                                if(((Map)datalist.get(j)).get(field)!=null){
                                    fieldVal=((Map)datalist.get(j)).get(field).toString().trim();
                                }
                                sqlFiled=field+"=";
                                if(((Map)datalist.get(j)).get(field)==null) {
                                  sqlValue="null";
                                }else if("1".equalsIgnoreCase(editType)) { //1|文本类型;2|数值类型;3|日期类型;4|大数据类型;{
                                    sqlValue=" '"+fieldVal+"' ,";
                                }else if("2".equalsIgnoreCase(editType)) {
                                    if("".equalsIgnoreCase(fieldVal)){
                                        sqlValue=" null ,";
                                    }else{
                                        sqlValue=" "+fieldVal+" ,";
                                    }
