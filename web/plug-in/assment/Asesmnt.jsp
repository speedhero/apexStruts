...
<%
    ...
    try {
       ...
       // JdbcTemplate jdbcTemplate = new JdbcTemplate(DataAccess.getDataSource());
        ...
        Map<String,Integer> prop= DataTblUtil.getTablePro(tableCode);//int isCheckBox,int isnumber,int isOperate
        customField=DataTblUtil.getCustomFiledStr(tableCode,prop.get("isCheckBox"),prop.get("isnumber"),prop.get("isOperate"));
...
<fieldset class="layui-elem-field site-demo-button" style="margin:10px;">
    ...
    <div style="float: right;width: 40px;">
             <a class="newlink_right" href="http://localhost:8080/apexstruts/plug-in/appraisal/AssSlider.jsp" target="_blank">
                 <i class="layui-icon layui-icon-screen-full" style="font-size: 30px; color: #1E9FFF;"></i>
            </a>
    </div>
</fieldset>
...
<%--        <button class="layui-btn layui-btn-sm" lay-event="getCheckData">获取选中行数据</button>
       ...
       <button class="layui-btn layui-btn-sm" lay-event="getCheckLength"><i class="layui-icon layui-icon-add-circle-fine" style="font-size:20px;font-weight:bold"></i>新增开门红</button>
        ...
        <button class="layui-btn layui-btn-sm layui-btn-danger" lay-event="import"><i class="layui-icon"></i>导入</button>--%>
        <%=toolBarButtons%>
...
<script type="text/javascript">
    ...
    var exportCloums=<%=exportCloums%>;
    var importCloums=<%=importCloums%>;
    ...
    function bindArea(table) {
        ...
        table.on('toolbar(assessmentTblFilter)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    ...
                    openForm(layer,data,'add','新增',obj);
                    break;
                //自定义头工具栏右侧图标 - 提示
                case 'LAYTABLE_TIPS':
        ...
        table.on('tool(assessmentTblFilter)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                openForm(layer,data,'detail','查看',obj);
            } else if(obj.event === 'del'){
                del(layer,data,obj);
            } else if(obj.event === 'edit'){
                openForm(layer,data,'edit','修改',obj);//1新增2修改3查看4删除
            }
