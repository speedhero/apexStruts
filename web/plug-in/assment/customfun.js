...
/*    data.unshift(exportfield)
    data = excel.filterExportData(data, importCloums);*/
    ...
    $.ajax({
        ...
        success: function (res) {
            if(res.code==0) {
                var data=res.data;
                data.unshift(exportfield)
                data = excel.filterExportData(data, exportCloums);
                excel.exportExcel(data, tableName+'.xlsx', 'xlsx');
            }else {
                var data=[];
                data.unshift(exportfield)
...
//编辑的方法
function  EidtUv(data,index,obj,layer,method) {
    var urladdr=url;
    var IDs=[];
    var datas=null;
    if(method=='add'){
...
//当前行数据,类型1新增 add ，2修改 edit 3 查看 detail
function openForm(layer,data,method,methodName,obj) {
    var btn=['确定', '取消'];
    if(method=="detail"){
        btn=[];
    }
    layer.open({
        ...
        ,skin: 'layui-layer-molv'
        ,area: ['60%', '80%']
        ,title: [tableName+'-'+methodName,'font-size:18px']
        ,btn: btn
        ,shadeClose: false
        ,shade: 0.4 //遮罩透明度
        ,maxmin: true //允许全屏最小化
        //,content:$("#window")  //弹窗路径
        ,content:'promotForm.jsp?tablecode='+tableCode+"&method="+method+"&data="+encodeURIComponent(JSON.stringify(data)) //弹窗路径 /plug-in/bank/assment/
       // , scrollbar: false, //屏蔽浏览器滚动条
        ,success:function(layero,index){
        },
        yes:function(index,layero){
            ...
            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
                ...
                EidtUv(field,index,obj,layer,method);
...
function deleteChecked(layer,checkData,obj) {
    layer.confirm('真的删除行么', function(index){
       var delList=[];
