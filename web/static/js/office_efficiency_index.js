		...
		if(rePortData.channelAvgTime>20){
			...
			if(rePortData.lineUpData.length>0){
				...
				if(t1>=3){
					...
					if(rePortData.popval<=0.3){
						reportContent="网点运营状态良好，规划合理，请继续保持";
					}else{
...
function initBaseInfo(){
   ...
   var dataDate=year + "年" + mon + "月" + date + "日" + weeks[week];
...
function loadPageData(){
	 ...
	 if(Object.keys(groupChannelInfo).length==0){
		 ...
		 if(undefined==groupChannelInfo || null==groupChannelInfo){
			 groupChannelInfo={"channelCode":"11228790","groupChannelCode":"3201001965930","channelName":"桐城农商银行@天水支行"};
	     }else{
